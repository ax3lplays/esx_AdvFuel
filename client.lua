...
Citizen.CreateThread(function()
	...
	while true do
		...
		if(isNearFuelStation and IsPedInAnyVehicle(GetPlayerPed(-1), -1) and not IsPedInAnyHeli(GetPlayerPed(-1)) and not isBlackListedModel() and not isElectricModel() and GetPedVehicleSeat(GetPlayerPed(-1)) == -1) then
			...
			if(isEmergencyModel()) then --Emergency
				if(menu) then
		...
		if(isNearFuelEStation and isEmergencyModel() and GetPedVehicleSeat(GetPlayerPed(-1)) == -1) then
			Info(settings[lang].openMenu)
			...
			if(menu) then
				...
				TriggerEvent("GUI:Option", settings[lang].confirm, function(cb)
					if(cb) then
						...
						TriggerServerEvent("essence:buygov", int, stationEmergencyNumber,false)
					else
		...
		if(isNearFuelEAStation and isEmergencyAirModel() and GetPedVehicleSeat(GetPlayerPed(-1)) == -1) then
			Info(settings[lang].openMenu)
			...
			if(menu) then
				...
				TriggerEvent("GUI:Option", settings[lang].confirm, function(cb)
					if(cb) then
						...
						TriggerServerEvent("essence:buygov", int, stationEmergencyAirNumber,false)
					else
...
function isNearEmergencyStation()
	local ped = GetPlayerPed(-1)
	local plyCoords = GetEntityCoords(GetPlayerPed(-1), 0)
...
function isNearEmergencyAirStation()
	local ped = GetPlayerPed(-1)
	local plyCoords = GetEntityCoords(GetPlayerPed(-1), 0)

	for _,items in pairs(emergencyair_stations) do
		if(GetDistanceBetweenCoords(items.x, items.y, items.z, plyCoords["x"], plyCoords["y"], plyCoords["z"], true) < 2) then
			return true
		end
	end
...
function isEmergencyModel()
	local model = GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1))))
	local isPl = false
	for _,k in pairs(emergency_model) do
		if(k==model) then
			isPl = true
			break;
		end
...
function isEmergencyAirModel()
	local model = GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1))))
	local isPl = false
	for _,k in pairs(emergencyair_model) do
		if(k==model) then
			isPl = true
			break;
		end
...
function isEmergencyBoatModel()
	local model = GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1))))
	local isPl = false
	for _,k in pairs(emergencyboat_model) do
		if(k==model) then
			isPl = true
			break;
		end
